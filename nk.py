...
def send_game_image(update: Update, context: CallbackContext):
    # Создаем поверхность для отображения игры
    screen_image = pygame.Surface((screen_width, screen_height))
    screen_image.blit(screen, (0, 0))
        
    # Сохраняем изображение в байтовый поток
    byte_io = io.BytesIO()
    pygame.image.save(screen_image, byte_io, "PNG")
    byte_io.seek(0)
        
    # Отправляем изображение в Телеграм
    update.message.reply_photo(photo=byte_io)
        
def start(update: Update, context: CallbackContext):
    update.message.reply_text('Привет! Добро пожаловать в игру. Введите /play чтобы начать.')

def play(update: Update, context: CallbackContext):
    show_welcome_screen()
    difficulty = choose_difficulty()
    game_loop(difficulty)
    send_game_image(update, context)
